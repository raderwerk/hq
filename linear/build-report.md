...
2. **Twee labelnamen botsen.** Linear eist dat labelnamen uniek zijn over de hele werkruimte, ook over groepen heen. `intern` staat in de spec zowel onder `dienst` als onder `facturatie`, en `wacht-op-mens` zowel onder `poort` als onder `schakelaar`. Alleen de eerste van elk paar bestaat nu. Gevolg: `dienst/intern` en `poort/wacht-op-mens` zijn er wel, `facturatie/intern` en `schakelaar/wacht-op-mens` niet, en de issues *Engagement: merklancering Raderwerk* en *Account: Raderwerk* missen daardoor hun facturatielabel. Hernoem één van elk paar in de spec, dan pakt een volgende run het vanzelf op.
